pipeline {
    ...
    stages {
        stage('Get EC2 Private IP') {
            steps {
                script {
                    FLASK_EC2_HOST = sh(script: "aws ec2 describe-instances --filters 'Name=tag:Name,Values=Flask-Server' --query 'Reservations[*].Instances[*].PrivateIpAddress' --output text", returnStdout: true).trim()
                    echo "Flask EC2 Private IP: ${FLASK_EC2_HOST}"
                }
            }
        }
        ...
        stage('Build & Push Flask Image') {
            steps {
                ...
                sh "docker login -u avihay1997 -p dckr_pat_UyFi28fTMFGMwRKl0Ch_pKoy1kw"
        ...
        stage('Deploy Flask on EC2') {
            steps {
                ...
                docker login -u ${DOCKER_USER} -p dckr_pat_UyFi28fTMFGMwRKl0Ch_pKoy1kw
