pipeline {
    ...
    stages {
        stage('Get EC2 Private IP') {
            steps {
                script {
                    def ubuntu = sh(script: "aws ec2 describe-instances --filters 'Name=tag:Flask-Server,Values=ec2_private_new_1' --query 'Reservations[].Instances[].PrivateIpAddress' --output text", returnStdout: true).trim()
